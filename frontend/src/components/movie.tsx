...
export function Movie(Base: typeof BackendMovie) {
  class Movie extends Routable(Base) {
    ['constructor']!: typeof Movie;

    @consume() static MovieList: typeof MovieList;
    ...
    @view() static Loader({id, children}: {id: string; children: (movie: Movie) => JSX.Element}) {
    ...
    @route('/movies/:id') @view() static Main({id}: {id: string}) {
      return (
        <this.Layout>
    ...
    @route('/movies/:id/edit') @view() static Editor({id}: {id: string}) {
      return (
        <this.Layout>
